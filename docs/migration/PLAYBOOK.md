...
Guia operacional para migrar os 7 projetos derivados (transitado-em-julgado, cuidari, ctvitrine, ctjuris, sorteiopix, ctfinance, spinmax) para os padrões atuais deste boilerplate (Laravel 13 + Inertia v3 + React 19 + Pest 5 + Larastan + TypeScript 6 + Node 24). Todos os paths de origem citados existem neste repositório e foram verificados no branch da harvest.
...
4. **Alvo parado.** O boilerplate reconciliado (branch `main` pós-harvest) é a referência congelada durante toda a rodada. Não se persegue alvo móvel: mudanças novas no boilerplate entram na próxima rodada, não no meio de uma migração. Exceção registrada: o alvo foi re-congelado em 2026-08-10 após atualização de dependências do boilerplate (Pest 5, Vitest 4, ESLint 10, TS 6, Node 24, pnpm 11.19) — permitido porque apenas a Fatia 0 do piloto havia rodado, e ela é imune a deps.

> **Nota (2026-09-08):** o tooling de agente — `CLAUDE.md` com `@AGENTS.md`, `config/boost.php`, `.claude/` (settings, hooks, rules por symlink, padrões de segurança) — entrou no alvo fora do re-congelamento de 2026-08-10, pela rodada agent-tooling ([#133](https://github.com/Simplify-Technology/boilerplate/issues/133): PRs #135, #137, #139, #142). Não fere o Princípio 4: não muda stack nem convenção de código, muda como o agente as carrega e as respeita. Derivado recebe esse kit como item da Fatia 2 (§3), nunca fora de fatia.
...
- **Copiar/adaptar:**
  ...
  - `.mise.toml` (Node 24, pnpm 11.19.0 — mesmas versões do CI) e `pnpm-workspace.yaml` com `minimumReleaseAge: 10080` (pacote npm só entra 7 dias após publicado — mitiga supply-chain attack de release recém-envenenada).
  ...
  - Atualizar toolchain de teste para o alvo: Pest `^5` (+ PHPUnit 13.2, migração sem breaking verificada no boilerplate), Vitest `^4`, ESLint 10, TS `~6.0` (NÃO TS 7 — typescript-eslint não suporta; remover `baseUrl` do tsconfig), Node 24 LTS, pnpm 11.19.
  ...
  - **Kit de agente (rodada agent-tooling #133):** `.claude/settings.json` (hooks `ci-gate.sh` no Stop e `guard-git.sh` no Bash, `enabledMcpjsonServers`, plugin `security-guidance` com `env.SECURITY_REVIEW_MODEL`), `.claude/hooks/*`, symlink `.claude/rules/conventions → ../../.ai/rules`, `.claude/security-patterns.json` + `.claude/claude-security-guidance.md` (só padrões cujos nomes existem no projeto; senão o subconjunto genérico), `config/boost.php` (guidelines do `claude_code` em `AGENTS.md`), `@AGENTS.md` na primeira linha do `CLAUDE.md`, e `.gitignore` com `.claude/.ci-gate/`, `.claude/settings.local.json`, `.claude/claude-security-guidance.local.md`. Depois de publicar o `config/boost.php`, regenerar o `AGENTS.md` com `php artisan boost:update --no-interaction --no-discover` (o Boost ≥ 2.5 põe a seção "Project Rules" e deixa de fixar versões no texto). **Pré-requisito:** `composer ci:check` e `pnpm ci:check` completos — o gate do Stop os chama; por isso é item desta fatia, não da 0. **Prova de carga:** sessão headless com hook `InstructionsLoaded` lendo um controller mostra `path_glob_match` das rules e `include` do AGENTS.md (roteiro nos PRs #135 e #137 do boilerplate).
- **Gate:** `composer ci:check` e `pnpm ci:check` verdes; hooks disparando localmente; primeiro PR do Dependabot abrindo sem ruído; kit de agente carregando (prova por `InstructionsLoaded`) e o gate do Stop bloqueando um teste vermelho de propósito.
...
- **Copiar/adaptar:**
  ...
  - **`.ai/rules/` + `CLAUDE.md`/`AGENTS.md` adaptados** — copiar `.ai/rules/index.md` e os arquivos de área aplicáveis, ajustando exemplos aos domínios do projeto. O kit de agente da Fatia 2 já projeta essas regras para o Claude Code (symlink) e para as demais ferramentas (seção "Project Rules" do AGENTS.md gerado pelo Boost ≥ 2.5); aqui só se adaptam os textos, sempre via `record-rule` para o `index.md` continuar gerado. *Na prática esta é a sub-fatia mais barata e mais rentável: antecipe-a para a Fatia 2* — agentes lendo as regras certas produzem todas as fatias seguintes no padrão.
...
- **`minimumReleaseAge` vs lockfile recém-atualizado.** Com `minimumReleaseAge: 10080`, um `pnpm update` que acabou de puxar versão publicada há menos de 7 dias falha na resolução seguinte. Sintoma: CI vermelho "sem motivo" logo após update de deps. Ou espere a janela, ou use a exceção pontual, ou fixe a versão anterior. **Forma mais dura vista no piloto (Fatia 2):** o pnpm ≥11.5 verifica o lockfile INTEIRO contra a política — inclusive no `pnpm run` local, via verify-deps — então ligar 10080 quebra na hora se QUALQUER entrada do lockfile for mais nova que a janela (lá: o piso de segurança `nanoid@3.3.18` da correção de advisories, com 3 dias de publicado). A válvula cirúrgica é `minimumReleaseAgeExclude` com versão exata (`nanoid@3.3.18`, suportado no pnpm ≥10.19), com a data de remoção anotada no próprio `pnpm-workspace.yaml` — não reverta o 10080 nem exclua o pacote inteiro. Corolário: se a Fatia 0 resolveu advisories via override, a Fatia 2 provavelmente vai precisar dessa exceção.
- **TS 7 (compilador Go) é latest no npm mas typescript-eslint exige <6.1** — pin `typescript@~6.0`; e o tsconfig não pode usar `baseUrl` (removido no TS 7). **Atenção:** o `package.json` do próprio boilerplate declara `typescript: ^6.0.3`, que contradiz esta prescrição — hoje é inofensivo (não existe 6.1 estável; o latest é 7.0.2), mas basta o 6.1 sair para o `^` violar o peer `>=4.8.4 <6.1.0` do typescript-eslint. Nos derivados use `~6.0.x` e registre a divergência em relação ao alvo no gap-report (feito no piloto, Fatia 2b).
- **Vitest 4 deixa de puxar `@types/node` — `global` no `setup.ts` quebra o `tsc`.** Com `types: ["vitest/globals"]` no tsconfig, a inclusão automática de `@types` fica desligada e os tipos de node entravam só de carona pela árvore do Vitest 3. No Vitest 4 não entram mais, e o `resources/js/test/setup.ts` (que faz `global.route = …` e `global.ResizeObserver = …`) falha com `TS2304: Cannot find name 'global'`. **O boilerplate não sente** porque a suíte dele importa o `vite.config.ts`, e o `vite/dist/node/index.d.ts` referencia `node` — ou seja, o derivado quebra num arquivo idêntico ao que passa lá. Correção mínima: `/// <reference types="node" />` no `resources/js/test/vitest.d.ts` (o `@types/node` já é devDependency direta). Vale para todo derivado na Fatia 2; não perca tempo suspeitando do `baseUrl` removido nem do subpath `@testing-library/jest-dom/vitest` (nenhum dos dois é a causa).
- **`composer update -W` para o Pest 5 arrasta o framework junto.** `--with-all-dependencies` libera a árvore inteira: no piloto subiu `laravel/framework` v13.21.1→v13.24.0 mais symfony, guzzle, carbon e commonmark — upgrade de framework clandestino dentro de uma fatia de tooling (Princípio 2 quebrado sem culpado no diff). Use **`--with-dependencies`** (`-w`): resolve o Pest 5/PHPUnit 13 com os `sebastian/*`, `phpunit/php-*` e paratest que eles exigem, e deixa o framework travado. Sem nenhum dos dois flags a resolução nem fecha (o `php-code-coverage` fica preso pelo lock). Custo do escopo estreito: alguma patch do ecossistema Pest pode ficar uma versão atrás do lock do boilerplate (piloto: `pest-plugin-laravel` v5.0.0 em vez de v5.0.1, mesma constraint `^5.0`) — anote no gap-report em vez de alargar para `-W`.
- **Prettier 3.9.6 reformata `resources/css/app.css`.** O `@source "../views"` vira aspas simples e o `format:check` fica vermelho logo após o bump. É a mesma linha que mudou no boilerplate no commit de re-freeze — rodar `pnpm format` é parte da fatia, não sintoma de problema.
- **O shim do mise não resolve em shell não-interativo.** Trocar `.mise.toml` para Node 24 não muda o `node`/`pnpm` de um shell `-c` (o `mise activate` depende de hook de prompt): o gate roda no Node antigo e você valida a coisa errada sem perceber. Rode os gates da fatia com `mise exec -- <cmd>` (ou shims explícitos). Detalhe que engana: o shim do corepack lê o `packageManager` do `package.json`, então o `pnpm -v` já responde a versão nova antes mesmo de o Node trocar.
...
- **`.ai/rules` não tem fiação própria — e não se copia sem fact-check.** O boilerplate não referencia `.ai/rules` em `CLAUDE.md`/`AGENTS.md` (lá a descoberta é pela convenção do `index.md` e pelo tooling do Boost), então "copiar CLAUDE/AGENTS adaptados" não traz a fiação junto: no derivado, adicione um ponteiro explícito no `CLAUDE.md` (ler a linha do `index.md` cujo glob casa com o arquivo) e NÃO edite `AGENTS.md`, que nos derivados do Boost tem invariante de cópia idêntica em três lugares (`.cursor/rules/laravel-boost.mdc`, `.github/copilot-instructions.md`). E as regras afirmam fatos do boilerplate que podem ser falsos no derivado (no piloto: helpers de persona de teste inexistentes, `preventStrayRequests` inativo, enum de domínio PascalCase, `authorize()` sem re-checagem num módulo local) — verifique cada afirmação contra o código real antes de gravar; regra falsa é pior que regra nenhuma. Regras de alvo (convenção que só chega em fatia futura) são legítimas, mas escritas como prescrição para código novo, nunca como descrição do presente. Nos arquivos de origem comum (RBAC etc.), a versão tipada do boilerplate embute fixes de comportamento colhidos na harvest (ex.: `unsetRelation` no refresh do cache de permissões, `is_active` no `LoginRequest`, generalização do `RoleFilterService`). Na fatia de análise estática, portar SÓ tipos/anotações e deixar cada fix para a fatia dona (4/6) — senão o Princípio 2 quebra em miniatura e o diff fica sem culpado. Exceção: correção exigida pelo próprio type-check (ex.: referência a relação inexistente, cast em aritmética de string) entra junto, documentada no gap-report.

- **`.ai/rules` não carrega em ferramenta nenhuma sozinho.** O Boost só escreve lá (`record-rule`); quem lê é (a) o Claude Code, se houver a projeção `.claude/rules/conventions → ../../.ai/rules` (symlink relativo; `paths:` do frontmatter vira regra por caminho), e (b) qualquer ferramenta que leia o AGENTS.md gerado pelo Boost ≥ 2.5, cuja seção "Project Rules" manda abrir `.ai/rules/index.md` antes de editar. AGENTS.md gerado por Boost anterior não tem a seção — regenerar. Prova: hook `InstructionsLoaded`.
- **O Claude Code lê `CLAUDE.md`, não `AGENTS.md`.** Sem `@AGENTS.md` na primeira linha do CLAUDE.md, as guidelines do Boost nunca entram no contexto — foi assim no boilerplate por meses e é assim nos 7 derivados. E o default do agente `claude_code` no Boost é escrever guidelines no próprio CLAUDE.md, **apendando** 280 linhas a cada `boost:update` quando não acha o marcador: publique `config/boost.php` com `agents.claude_code.guidelines_path = 'AGENTS.md'` antes de rodar o comando.
- **`.claude/settings.local.json` só está protegido pelo gitignore global de uma máquina.** O `.gitignore` de projeto precisa de `.claude/settings.local.json`, `.claude/.ci-gate/` e `.claude/claude-security-guidance.local.md`; senão o júnior versiona a allow-list de permissões dele. E o inverso: ctjuris ignora `.claude/` inteiro — o kit não pode ser versionado lá até liberar o diretório.
- **Hook do Claude Code em worktree usa `cwd`, não `CLAUDE_PROJECT_DIR`.** O segundo fica no checkout principal quando o agente entra num worktree; scripts de hook que avaliam a árvore leem o `cwd` do JSON de entrada (o `ci-gate.sh` do kit já faz isso).
- **Hook de Stop precisa de anti-loop.** Bloquear o encerramento com exit 2 faz o agente continuar; sem trava, teste vermelho pré-existente vira loop. O kit libera a segunda vez na mesma árvore, e o Claude Code manda `stop_hook_active` e impõe teto de 8 bloqueios. Vermelho fora do escopo: o agente relata, não contorna.
- **`security-guidance` custa modelo por turno e por commit, e o default é Opus.** Fixe `env.SECURITY_REVIEW_MODEL` no settings compartilhado; Python < 3.10 só degrada o reviewer agentic para single-shot. Os padrões built-in não têm PHP — o `.claude/security-patterns.json` do kit cobre; e o `fnmatch` do plugin **não** casa `dir/**/*.php` em arquivo direto no diretório (use `dir/**`).
- **A guarda dos hooks do git tem um falso positivo conhecido.** Heredoc no Bash que cite `--no-verify` ou a variável de skip (documentação, mensagem de commit) é bloqueado; escreva o arquivo com a ferramenta de edição e passe `-F`.

...
**Progresso:** 12/70 células (17%) · BACKLOG: **52 aplicados (A1, A3, A6, D2, D3, D4, D5, E17, E2, E13, F1, F5, F22, F42, F35, F23, S1, S2, S4, S5, C4, S3, E6, E20, E14, E15, E30, E22, E24, E27, E29, E18, E23, E25, E21, E12, E28, F32, V6T13, V6T14, V6S-1, F2, F3, F9b, E12+E21-visual, V6F-4, V6T10, V6D-3, V6D-2, V6F-1, V6S-2, V6F-6)**, 1 realocado (A2), **~110 aplicáveis** (8 de dim. 1–3 · 27 de dim. 5 · **69 de dim. 6: F1–F42 + secagem** · 11 da dim. 1 do spinmax), 7 adiados, **11 rejeitados**, 9 sem veredito (dim. 4), **4 achados internos (C1, C2, C3, C4)**, **2 `[dep-nova]` novos** (`jest-axe`, `knip`). Decisão do dono sobre o canal de flash: **resolvida em 2026-08-11 (nativo)**.
...
### Próxima unidade sugerida (após V6S-2+V6F-6)

A fila P da dimensão 6 do ctvitrine, separada por quem toca `js.md`:

1. **Enquanto #123 estiver aberto — só o que não passa por `js.md`:** **V6S-4** (`SheetHeader` fora do `SheetContent`: 2 frases `sr-only` permanentes no corpo — a11y, medido na lib Radix instalada; regra cabe em `js.md`… ⚠️ conferir se dá para travar só com teste de componente e sem regra nova) · **V6P-1** (`min-w-0` no `SidebarInset`, uma classe) · **V6F-7 (metade)** (poda do ramo `variant="header"` morto em `app-shell.tsx`/`app-content.tsx`; a regra de código morto já existe desde E27+E29).
2. **Depois de #123 mesclar:** **V6D-11** (3 idiomas de spinner, 15 infratores — fecha o follow-up do E28) · **V6P-6** (`appearance-tabs` sem papel/estado ARIA e em inglês, junto do F41) · **V6P-5** (SVG de marca sem a11y).
3. **V6T5/F6 (Radix × `--color-background`)** — decisão do dono pendente (ver 15ª). **F14** (`theme-color` que segue a troca de tema). **F20** quando a leva secar.

Alternativas de Fase A: **dimensão 1 (Segurança) do cuidari ou do ctvitrine**, ou **inventário do sorteiopix** (matriz em 12/70). Fatia de deps: só depois de o dono mesclar #126/#127/#128 (o que sobra é o pin `typescript ~6.0.x` e a decisão sobre pnpm 12).

...
- **Situação:** Fase 0 concluída · varredura em andamento (12/70 células) · **34 fatias MESCLADAS** (A1, A3, A6, D2, D3, D4, D5, E17, E2+E13, F1, F5, F22, F42+F35, F23, S1, S2, S4, S5, C4, S3, E6+E20, E14+E15, E30, E22+E24, E27+E29, E18+E23+E25, E21+E12, E28, F32, E26, V6S-1+V6T14+V6T13, F2+F3+F9b, F38) · **3 PRs abertos: [#123](https://github.com/Simplify-Technology/boilerplate/pull/123) (E12+E21 metade visual) · [#130](https://github.com/Simplify-Technology/boilerplate/pull/130) (V6S-2+V6F-6 — CSS morto em `app.css`) · [#132](https://github.com/Simplify-Technology/boilerplate/pull/132) (V6F-7+V6P-1+V6P-2 — shell sem ramo morto)**
...
**Progresso:** 12/70 células (17%) · BACKLOG: **55 aplicados (A1, A3, A6, D2, D3, D4, D5, E17, E2, E13, F1, F5, F22, F42, F35, F23, S1, S2, S4, S5, C4, S3, E6, E20, E14, E15, E30, E22, E24, E27, E29, E18, E23, E25, E21, E12, E28, F32, V6T13, V6T14, V6S-1, F2, F3, F9b, E12+E21-visual, V6F-4, V6T10, V6D-3, V6D-2, V6F-1, V6S-2, V6F-6, V6F-7, V6P-1, V6P-2)**, 1 realocado (A2), **~110 aplicáveis** (8 de dim. 1–3 · 27 de dim. 5 · **69 de dim. 6: F1–F42 + secagem** · 11 da dim. 1 do spinmax), 7 adiados, **11 rejeitados**, 9 sem veredito (dim. 4), **5 achados internos (C1, C2, C3, C4, C5)**, **2 `[dep-nova]` novos** (`jest-axe`, `knip`). Decisão do dono sobre o canal de flash: **resolvida em 2026-08-11 (nativo)**.
...
| **F38** (V6F-4 + V6T10/V6D-3 + V6D-2/V6F-1) — fix(views): `public/` e o `<head>` só carregam o que se usa | [#124](https://github.com/Simplify-Technology/boilerplate/issues/124) | `124-harvest-v2-poda-public-head` | ✅ 6 casos / 23 asserções novos (back 416/2046 → **422/2069**) + **7 mutações** | ✅ ambos exit 0 (422/2069 · 42/386), também no `pre-push` | [#125](https://github.com/Simplify-Technology/boilerplate/pull/125) | ✅ **MESCLADO** 2026-09-04 17:29 UTC |
| **V6S-2+V6F-6** — fix(css): `app.css` deixa de carregar 29 linhas que não pintam nada | [#129](https://github.com/Simplify-Technology/boilerplate/issues/129) | `129-harvest-v2-poda-css-morto-app` | ✅ 2 casos novos (back 422/2069 → **424/2071**) + **4 mutações** (3 falham onde devem, o par coerente passa) | ✅ ambos exit 0 (424/2071 · 42/386), também no `pre-push` | [#130](https://github.com/Simplify-Technology/boilerplate/pull/130) | 🟡 **ABERTO** 2026-09-08 |
| **V6F-7+V6P-1+V6P-2** — fix(shell): o shell só tem o caminho da sidebar, `min-w-0`, guarda de dependência sem importador | [#131](https://github.com/Simplify-Technology/boilerplate/issues/131) | `131-harvest-v2-shell-sem-ramo-morto` | ✅ 6 testes novos (front 42/386 → **44/392**) + **7 mutações** | ✅ ambos exit 0 (422/2069 · 44/392), também no `pre-push` | [#132](https://github.com/Simplify-Technology/boilerplate/pull/132) | 🟡 **ABERTO** 2026-09-08 |
...
### Reconciliação da 3ª unidade da 16ª invocação (2026-09-08)

Feita ANTES de qualquer ação, porque a data mudou entre a 2ª e a 3ª unidade.

- **PR #125 (F38) confirmado MESCLADO** pelo dono em 2026-09-04 17:29 UTC; `origin/main` em **`f36fb01`**. **PR #123 (E12+E21 visual) segue ABERTO** — e é ele que decide a fila: toda fatia que toque `.ai/rules/js.md` nasce em conflito com ele (trap estrutural registrada), então a 3ª unidade foi escolhida entre as P que **não** passam por `js.md`.
- **Três PRs de dependabot ABERTAS** — #126 (composer minor/patch), #127 (actions), #128 (npm minor/patch). São do dono; não abrir fatia de deps concorrente sobre os mesmos lockfiles. Anotado também: o `corepack pnpm` avisa **pnpm 12.3.4 disponível** (o STATE de 2026-08-11 registrava pnpm 12 em RC) — major, **não adotar** sem validar corepack/hooks/CI (PLAYBOOK §4); vira linha da próxima fatia de deps, não desta.
- **Drift:** ctfinance em **`583e721`** (era `5865134`; pin `b8c6d57`). spinmax `ee7962a`, ctvitrine `f9f17f6`, cuidari `0bb97f8` (iguais à reconciliação anterior). sorteiopix, ctjuris, tej idênticos aos pinados. Pins inalterados.
- **Worktree principal:** estava na branch `124-…` (já mesclada) com o WIP do dono intocado em `docs/migration/*`. `git switch -c` a partir de `origin/main`, `git add` por caminho.

### Fatia V6S-2 + V6F-6 (metade "apagar") — aplicada (2026-09-08, 3ª unidade da 16ª invocação)

Issue [#129](https://github.com/Simplify-Technology/boilerplate/issues/129) · PR [#130](https://github.com/Simplify-Technology/boilerplate/pull/130) **aberto, aguardando merge do dono**. Commit `354c32c` sobre `origin/main` = `f36fb01`. 2 casos novos em `tests/Unit/Views/SafeAreaOptInTest.php` (back 422/2069 → **424/2071**), **4 mutações** (3 falham onde devem, o par coerente passa), os dois gates exit 0 (também no `pre-push`). Não toca `js.md`.

**O que entrou.** Poda das 28 linhas do bloco "Global scrollbar for dialogs and modals in dark mode" (o `custom-scrollbar` está na className base de todo `DialogContent`, então o seletor descendente não casa nada e o resto era repetição na mesma especificidade) e da linha `padding: env(safe-area-inset-*)` do `body` (sem `viewport-fit=cover` vale `0px`). Teste de contrato: inset só com opt-in, opt-in só com consumidor, comentários ignorados. `.ai/rules/css.md`: frase na seção da #108 + seção nova sobre safe-area. **Evidência de zero efeito:** diff do CSS compilado — 825.430 → 824.749 B (−681 B), removidos exatamente os 5 blocos `.dark [data-slot=dialog-content]…` e, dentro de `@layer base`, a única declaração de `padding`; nenhum bloco novo.

**Dois fatos de método, os dois sobre teste que passa mentindo:**

1. **⚠️ No Pest, o segundo argumento de `toContain` é OUTRO needle, não a mensagem.** `expect($x)->not->toContain('a', 'mensagem…')` passa sempre que a mensagem não está em `$x` — a primeira versão do teste passou VERDE numa mutação que devia falhar (M2) e FALHOU no par coerente que devia passar (M3), e só a tabela de mutações denunciou. `InlineThemeBackgroundTest.php:174-175` já tinha o comentário avisando; eu li e escrevi errado mesmo assim. Forma certa: `expect(str_contains($x, 'a'))->toBe(true|false, 'mensagem')`. **Toda mutação "deve PASSAR" vale tanto quanto as "deve FALHAR": foi a M3 que expôs o `toContain`.**
2. **Comentário em prosa conta como consumidor se o teste lê o arquivo cru.** O `app.css` ganhou um comentário explicando por que NÃO há `env(safe-area-*)`, e o teste (que procura `safe-area-` em `resources/`) ficou vermelho por causa da explicação. Mesma armadilha do `themeStyleBlock()` (um comentário Blade citava a tag). Todo teste de fonte que procura um token em `resources/` precisa tirar `/* */`, `{{-- --}}` e `//` antes — e a doc do teste tem de dizer que a prosa não consome.

**Escopo deliberadamente não tocado:** rolagem estilizada no `SheetContent` (regra nova, não esta); a metade "ativar" do V6F-6 vira fatia própria só com página pública ou PWA instalável, no idioma atual (`max()` + `safe-area-max-inset-*` com fallback, no elemento que encosta na borda, e tratamento dos `fixed`).

### Fatia V6F-7 + V6P-1 + V6P-2 — aplicada (2026-09-08, 4ª unidade da 16ª invocação)

Issue [#131](https://github.com/Simplify-Technology/boilerplate/issues/131) · PR [#132](https://github.com/Simplify-Technology/boilerplate/pull/132) **aberto, aguardando merge do dono**. Commit `9bdcb8d` sobre `origin/main` = `f36fb01` (independente de #123 e #130). 6 testes novos (front 42/386 → **44/392**), **7 mutações, todas mordem**, os dois gates exit 0 (também no `pre-push`), `pnpm types` limpo. Não toca `js.md`.

**O que entrou.** `AppShell`/`AppContent` sem a prop `variant` e sem o ramo `header` (o default derrubava a árvore sem `SidebarProvider` e trocava o `<main>` do skip-link); `min-w-0` no call-site (`AppContent`), não no vendorizado; `appearance-dropdown.tsx` apagado (0 importadores, as únicas strings de UI em inglês); guarda `test/lib/dependency-importers.test.ts` (quote-agnóstica, tooling do starter por config/composer, dívida datada amarrada); `test/support/sources.ts` extraído e adotado por `focus-ring` e `link-button-nesting`; `.ai/rules/dependencies.md` novo (escopo `package.json`/`composer.json`) + linha no `index.md`.

**Achado interno novo — C5:** `@headlessui/react` está em `dependencies` com **zero importadores** desde o starter kit; nenhuma célula tinha visto (a V6P-2 só apontou o `navigation-menu`). Registrado no BACKLOG; sai com o `navigation-menu` na fatia de deps, depois de #128 mesclar — a guarda cobra pela dívida datada.

**Três fatos de método:**

1. **Prettier sobre `.ai/rules/index.md` realinha a tabela inteira** (49 linhas de diff para 1 linha de conteúdo). O `lint-staged` não cobre `.ai/` (só `resources/**`), então não há formatação canônica ali: **não rodar `prettier --write` em `.ai/rules/*.md`**; editar à mão e conferir o diff. Corrigido com `--amend` + `--force-with-lease` antes de abrir o PR (branch própria, sem PR ainda — o único momento em que reescrever é barato).
2. **A dívida datada só funciona com simetria nos DOIS sentidos** — "entrada existe no `package.json`" **e** "entrada ainda sem consumidor". A M5 (tirar `@headlessui` da dívida sem tirar do `package.json`) e a M6 (pôr `react` na dívida) precisam falhar separadamente; uma asserção só cobriria uma delas.
3. **A mutação que reproduz a trap vale mais que a que quebra o código.** M7 (regex só de aspas simples) é uma mutação do próprio teste, e é a que prova que a guarda não nasce com a receita errada do caçador. Quando a lente registra "a prova publicada está errada", a fatia deve ter uma mutação que reproduza exatamente aquele erro.

### Próxima unidade sugerida (após V6F-7+V6P-1+V6P-2)

A fila P **sem `js.md`** secou nesta invocação (V6S-4 é a única que resta, e a regra dela cabe melhor em `js.md`). Com **três PRs abertos** (#123, #130, #132), a próxima invocação deve **reconciliar primeiro**: se #123 mesclar, destrava a fila de `js.md`:

1. **V6D-11** (3 idiomas de spinner, 15 infratores — fecha o follow-up do E28) · **V6P-6** (`appearance-tabs` sem ARIA e em inglês, com F41) · **V6P-5** (SVG de marca sem a11y) · **V6S-4** (`SheetHeader` fora do `SheetContent`) · migrar `state-color-consumers.test.ts` para `test/support/sources.ts`.
2. **Fatia de deps** (depois de #126/#127/#128 mesclarem): remover `@radix-ui/react-navigation-menu` + `@headlessui/react` (apagar as duas entradas da dívida no mesmo commit), pin `typescript ~6.0.x`, decisão sobre pnpm 12 (major, validar corepack/hooks/CI), re-levantar `outdated`/`audit`.
3. **V6T5/F6** — decisão do dono pendente. **F14** (`theme-color`). **F20** quando a leva secar.

Se nada mesclar: **Fase A** — dimensão 1 (Segurança) do cuidari ou do ctvitrine, ou inventário do sorteiopix (matriz em 12/70). Antes de disparar workflow longo: `caffeinate -i -t <segundos>` (5ª pegadinha).

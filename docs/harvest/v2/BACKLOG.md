...
### ~~E14 + E15~~ ✅ APLICADO · PR [#94](https://github.com/Simplify-Technology/boilerplate/pull/94) · `[guard-rail]` + `[absorver]` `EmptyState`: HTML inválido e beco sem saída · P · risco baixo · **1 PR, 1 arquivo**
...
### E27 + E29 · ✅ **APLICADO** (PR [#100](https://github.com/Simplify-Technology/boilerplate/pull/100), 2026-08-12) · `[guard-rail]` + `[absorver]` código morto do starter kit que finge cobertura · P · risco baixo · **1 PR**

- **Confirmado na aplicação:** as quatro alegações de orfandade bateram exato, e a refutação que esta entrada já trazia ("hoje **não quebra**, é armadilha latente") era o tempo verbal certo. O `ui/navigation-menu.tsx` entrou na poda junto — a cadeia órfã inteira caiu.
- **A catraca do F5 cobrou sozinha:** a asserção "não carrega entrada obsoleta na lista de mortos conhecidos" falhou assim que o arquivo isento sumiu, exigindo esvaziar `MORTOS_CONHECIDOS`. Modelo a repetir: toda lista de exceção nasce com a asserção que exige removê-la.
- **Achado novo, fora do escopo previsto:** o mock de `ResizeObserver` em `test/setup.ts` **nunca foi construtível** (`vi.fn().mockImplementation(() => ({...}))` com arrow). Nenhum teste tinha alcançado porque nenhum renderizava flutuante do Radix. **Isto estava bloqueando silenciosamente toda fatia de UI que precise abrir dropdown/popover/tooltip — inclusive o E28.** Corrigido nesta fatia.
- **Terceiro defeito do banner, não previsto pela entrada:** `bg-teal-500` + `text-white` = **2.42:1** (oklch do Tailwind 4.3), reprova AA. Foi para `bg-teal-700` (**5.39:1**), com o teste travando o **piso** e não o tom (mutação para `-800` passa; `-500` e `-600` falham).
- **Escopo mantido fora, como previsto:** guarda de duplo-POST (o Inertia já aborta em voo) e remoção de `@radix-ui/react-navigation-menu` do `package.json` (lockfile; é trabalho do `knip`, `[dep-nova]`).
...
### ~~E28~~ · ✅ **APLICADO** (PR [#106](https://github.com/Simplify-Technology/boilerplate/pull/106), 2026-08-13) · `[absorver]` `loading`/`aria-busy` no `Button` · M · risco médio
...
- **Como saiu (2026-08-13):** os 4 itens do escopo entraram. **Terceira divergência, descoberta na aplicação e não prevista aqui:** o `aria-hidden="true"` que a origem escreve no ícone é **redundante** — `lucide-react@1.28.0` já o injeta em ícone sem prop de a11y própria (`dist/cjs/lucide-react.js:92`, guarda `!children && !hasA11yProp(rest)`). Ele foi removido e o teste ficou; a linha era código que nenhuma mutação mata. Consequência lateral verificada: os 6 `LoaderCircle` das páginas de auth **já** estão ocultos pelo mesmo default.
- **Follow-up desbloqueado (não aberto):** migrar os 8 spinners artesanais **de botão** (`add-permission-dialog:211`, `assign-role-user:200`, `delete-confirmation-dialog:239`, `user-form:369`, `roles:208`, `settings/password:201`, `settings/profile:211`, `users/permissions:167`) e os 6 `LoaderCircle` de auth para `loading`. **É essa fatia que deve carregar a catraca de contagem decrescente** — criá-la agora travaria os 14 de uma vez, o mesmo erro que o F20 já tem anotado. Atenção: `search-bar.tsx:110` e `filter-panel.tsx:169` usam o mesmo `<div>` artesanal mas **não são botão** (são indicador inline de busca) e ficam fora da conta — catraca que os incluir cobra migração que não existe.
...
| ~~**E12+E21** (`delete-confirmation-dialog.tsx`)~~ ✅ PR [#123](https://github.com/Simplify-Technology/boilerplate/pull/123) (2026-09-04) | 35 + 14 literais → tokens de estado; os dois arquivos na lista de limpos; 11 testes de DOM travam o mapa severidade → token. **Divergências:** chip do item foi para o trio `info` (não `primary`: 2,92:1 no escuro), botão de aviso é `bg-warning text-warning-foreground` sobre a variante `default` (sem variante nova no `Button`) | entrou com o F3 mesclado (#121) |
...
| F20 | **422 utilitários de cor literal em 35 arquivos** de produção — os primitivos estão limpos, os consumidores é que furam | guard-rail M | catraca com contagem decrescente; **depois** dos PRs de cor, senão trava todos. **Resíduo medido na #123 (2026-09-04):** os 6 `*-info-dialog.tsx` restantes (`appearance`, `password`, `profile`, `role`, `user-info`, `user-show-info`) carregam 24 `iconColor` em `cyan`/`purple`/`blue`/`yellow` — cor de marca e de seção, não de estado; entram aqui ou no F7, nunca no `state-color-consumers` |

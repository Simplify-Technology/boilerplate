...
## Trilha de auditoria: decida por feature e registre eventos de segurança à mão
Feature que toca estado crítico de negócio, ciclo de vida de conta, cargos/permissões, personificação ou qualquer ação sensível ganha trilha de auditoria — decida isso explicitamente ao planejar e diga na entrega se a trilha entrou ou por que não; em dúvida, pergunte antes de fechar. Mudança de estado de model vem do trait `LogsActivity` (ver models.md); o que não é ciclo de vida de Eloquent — início e fim de personificação, ações de segurança — é registrado à mão com `activity('security')->performedOn($alvo)->causedBy($ator)->event('snake_case')->withProperties([...])->log('snake_case')`, com `type`, `scope => 'security'`, o contexto do request em `properties.request` (`url`, `ip_address`, `user_agent`) e os metadados do evento num objeto aninhado próprio (`properties.impersonation`), como fazem `app/Listeners/LogImpersonateStarted.php` e `LogImpersonateStopped.php`. Sob personificação o causer é o humano real, nunca a persona: o log automático já sai assim porque o `AppServiceProvider` registra `App\Resolvers\ActivityCauserResolver::resolve()` (que lê `ImpersonationService::getOriginalUser()`), e os listeners passam o ator original em `causedBy()`; fluxo novo que gera activity sob personificação prova `causer_id`/`causer_type` em teste (`tests/Feature/ImpersonateTest.php`).

...
## `toast.promise` não herda severidade nenhuma — passe `toastPromiseOptions`

O `<Toaster>` NÃO alcança `ariaProps`: `createToast` grava `{ role: "status", "aria-live": "polite" }` no próprio objeto do toast e o merge do provider é `{ ...defaults, ...defaults[type], ...toast }`, com o toast por último. Chamada de duas posições (`toast.promise(promise, mensagens)`) entrega, portanto, erro `polite`, sem borda de variante e com o disco no default da lib — `#61d345`, que dá 1.92:1 sobre o card claro. O terceiro argumento é o único canal, e ele já existe pronto: `toastPromiseOptions` de `lib/toast-config.ts`, que reaproveita as mesmas opções das chamadas diretas em vez de abrir uma segunda fonte de verdade. `resources/js/test/lib/toast-promise-call-sites.test.ts` cobra isso em todo call site do `resources/js`.
...
## Atalho global de teclado não intercepta tecla dentro de campo, e não é invisível

Handler de `keydown` registrado em `window` alcança também o que a pessoa está digitando: o `preventDefault()` dele come a tecla nativa do campo. O caso medido é o `Cmd/Ctrl+B` da sidebar — no macOS, Ctrl+B é o binding do sistema para "mover o cursor um caractere à esquerda" dentro de `<input>`/`<textarea>`, e o handler vendorizado do shadcn (`ui/sidebar.tsx`) cancelava a tecla em toda página autenticada. Todo handler global faz early-return por `isTypingTarget(event.target)` (`lib/keyboard.ts`) **antes** de `preventDefault()`; a guarda checa `isContentEditable` (herdada, é a correta no browser) **e** o `closest('[contenteditable]:not([contenteditable="false"])')`, porque o jsdom não implementa a propriedade e sem o fallback o caso não seria testável. A checagem do modificador continua sendo `metaKey || ctrlKey`: as duas teclas valem nas duas plataformas. E atalho que ninguém descobre não existe — o controle correspondente declara `aria-keyshortcuts` com as duas formas (`"Control+B Meta+B"`, estático, renderiza no SSR) e mostra a dica numa tooltip, escrita na convenção da plataforma por `shortcutLabel()`, que lê `navigator` só sob demanda e depois da montagem. `resources/js/test/lib/keyboard.test.ts` e `resources/js/test/components/ui/sidebar-shortcut.test.tsx` travam os dois lados.

## Cor de estado no JSX vem do token do papel, não da paleta

Sucesso, aviso, informação e erro têm dois conjuntos de token com trabalhos diferentes, e o erro clássico é misturá-los no call site. **Sólido** (`bg-X text-X-foreground`) é botão, badge e disco de ícone — o rótulo é o do par, nunca `text-white`: no escuro o sólido é um tom claro com texto navy, e `text-white` em cima dava 2.69:1 (era o que `ui/button.tsx` e `ui/badge.tsx` escreviam). **Texto em cor de estado** sobre canvas, card ou item de menu é `text-state-X` (o `fg` do trio, medido nos dois temas contra as três superfícies); `text-X` cru é a cor sólida e reprova AA como texto no claro, e `text-X-foreground` solto é branco sobre branco — foi assim que `<Alert variant="destructive">` e o item `destructive` do `DropdownMenu` ficaram invisíveis no tema claro. **Callout suave** (aviso de conta, alerta de formulário, quadro de consequência) é `state-X-soft`, que aplica fundo, borda e texto de uma vez; `bg-state-X`/`border-state-X` existem para compor. Literal da paleta (`text-red-600 dark:text-red-400`, `bg-amber-50`, `border-emerald-200`) não entra em código novo de estado: ele fica fora de qualquer contrato de contraste, e o teste de fonte (`resources/js/test/styles/state-color-consumers.test.ts`) mantém uma lista de arquivos que já migraram e não voltam. Item de menu destrutivo usa `variant="destructive"` do primitivo em vez de repetir as classes. Ícone do lucide dentro de um callout `state-X-soft` não leva classe de cor: o SVG desenha com `stroke="currentColor"` (é o default de `lucide-react`) e herda a do callout — classe própria só desamarra o ícone do par medido. Severidade de call site (`danger`/`warning`) vira token por um mapa fechado (`calloutBySeverity` em `delete-confirmation-dialog.tsx`), não por ternário com literal em cada ramo.

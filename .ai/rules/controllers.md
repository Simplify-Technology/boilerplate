...
Autorize em camadas: middleware can:<permission> no grupo de rota, re-autorização dentro do controller ($this->authorize() ou Gate::authorize()) e authorize() com $this->user()->can() nos Form Requests de escrita. Nunca dependa de uma camada só. Controller que CONCEDE permissão ou cargo tem uma pergunta a mais, que nenhuma dessas camadas faz: "o ator tem isso para dar?" — passe o conjunto validado na autorização (`Gate::authorize('mutatePermissions', [$user, $nomes])`) e veja `.ai/rules/policies.md`.

...
7. **PR gigante é proibido**; fatia pequena, revisável, reversível. `SKIP_GIT_HOOKS` nunca para contornar gate vermelho — se o hook falhar por AMBIENTE (pnpm/mise fora do PATH), rode os dois `ci:check` manualmente (`corepack pnpm` / `mise exec`) e só então `SKIP_GIT_HOOKS=1` naquele push, registrado no STATE.md.
